...
public class Menu {
	...
	private void userSettings() {
		...
		case 1:
			...
			System.out.println("打开：T | 关闭 F | 返回 R");
			...
			if (iiv.isNumber(str)) {
				...
				if (bike == null) {
					...
					if (result == 1) {
						...
						System.out.print("该车辆已经被租出,请重新输入单车Id："); // 其实执行不到你
					} else if (result == -1) {
						System.out.print("该车辆已经损坏,请重新输入单车Id：");
					}
				}
			} else {
				System.out.println("输入不合法,请重新输入：");
